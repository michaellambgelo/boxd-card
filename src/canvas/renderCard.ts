...
import { TYPE_SCALE, cardFont } from './typeScale'
...
// Poster-grid text sizes now live in TYPE_SCALE (./typeScale); the line heights
// below stay here because they are layout metrics, not type sizes.
...
// TMDB attribution logo. SVG viewBox is 273.42 × 35.52 (≈7.7:1 aspect ratio).
// Drawn in the footer when `usedTmdb` is true. Height matches footer text size.
const TMDB_LOGO_H = 20
const TMDB_LOGO_W = Math.round(TMDB_LOGO_H * 273.42 / 35.52)  // ≈ 154
const TMDB_LOGO_PAD = 16  // gap between TMDB logo and "generated by Boxd Card" text

async function drawTmdbLogo(ctx: CanvasRenderingContext2D, rightX: number, centerY: number) {
  try {
    const img = await loadImage(tmdbLogoUrl)
    // rightX is the right edge of the logo; position x so the logo ends at rightX
    ctx.drawImage(img, rightX - TMDB_LOGO_W, centerY - TMDB_LOGO_H / 2, TMDB_LOGO_W, TMDB_LOGO_H)
  } catch { /* skip attribution logo on load failure */ }
}
...
// ── Tag pills ────────────────────────────────────────────────────────────────
const TAG_FONT     = cardFont(TYPE_SCALE.tag)
const TAG_PILL_H   = 56
const TAG_PAD_X    = 20
...
function measureReviewRows(
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (showOverview && film.overview) {
      if (!firstMeta) contentH += cfg.metaGap
      measureCtx.font = cardFont(cfg.reviewFs, 'italic')
    ...
    if (film.reviewText) {
      ...
      measureCtx.font = cardFont(cfg.reviewFs)
...
function drawStatsHeader(
  ...
  if (statsTitle) {
    ...
    ctx.font = cardFont(TYPE_SCALE.statsTitle, 'bold')
  ...
  if (statsSubtitle) {
    ...
    ctx.font = cardFont(TYPE_SCALE.statsSubtitle)
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
...
async function renderStatsSummary(
  ...
  const { username, footerAvatarDataUrl, showShareIcon, backdropDataUrl, usedTmdb } = options
  ...
  for (let i = 0; i < stats.length; i++) {
    ...
    ctx.fillStyle = TEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.statsSummaryValue, 'bold')
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
    ctx.fillText(stats[i].value, cx, cy)

    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.statsSummaryLabel)
...
async function renderByWeekChart(
  ...
  const { username, footerAvatarDataUrl, showShareIcon, backdropDataUrl, chartData, usedTmdb } = options
  const cardHeight = layoutCardHeight(layout)
  ...
  if (weeklyFilms.length > 0) {
    ...
    ctx.font = cardFont(TYPE_SCALE.byWeekAxis)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
  ...
  if (summaryNumbers.length > 0) {
    ...
    for (let i = 0; i < summaryNumbers.length; i++) {
      ...
      ctx.fillStyle = TEXT_COLOR
      ctx.font = cardFont(TYPE_SCALE.byWeekTotal, 'bold')
      ctx.textAlign = 'center'
      ctx.textBaseline = 'top'
      ctx.fillText(summaryNumbers[i].value, cx, y)
      ctx.fillStyle = SUBTEXT_COLOR
      ctx.font = cardFont(TYPE_SCALE.byWeekTotalLabel)
  ...
  if (dayOfWeek.length > 0) {
    ...
    for (let i = 0; i < dayOfWeek.length; i++) {
      ...
      ctx.fillStyle = DIM_COLOR
      ctx.font = cardFont(TYPE_SCALE.byWeekTick)
      ctx.textAlign = 'center'
      ctx.textBaseline = 'top'
...
async function renderBreakdownCard(
  ...
  const { username, footerAvatarDataUrl, showShareIcon, backdropDataUrl, breakdownData, usedTmdb } = options
  ...
  for (const r of ratios) {
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.breakdownSectionLabel)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
    ...
    ctx.fillStyle = TEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.breakdownPieCenter, 'bold')
    ctx.textBaseline = 'middle'
    if (prim > 40) {
      ctx.textAlign = 'left'
  ...
  if (bd.ratingSpread.length > 0) {
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.breakdownLegend)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
    ...
    for (let i = 0; i < bd.ratingSpread.length; i++) {
      ...
      ctx.fillStyle = DIM_COLOR
      ctx.font = cardFont(TYPE_SCALE.breakdownSpreadTick)
      ctx.textAlign = 'center'
      ctx.textBaseline = 'top'
      ...
      if (bd.ratingSpread[i] > 0) {
        ctx.fillStyle = SUBTEXT_COLOR
        ctx.font = cardFont(TYPE_SCALE.breakdownSpreadCount)
        ctx.textAlign = 'center'
  ...
  if (bd.watchlist.watched > 0 || bd.watchlist.added > 0) {
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.breakdownWatchlist)
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
    ctx.fillText(
...
async function renderBarChartCard(
  ...
  const { username, footerAvatarDataUrl, showShareIcon, backdropDataUrl, barChartData, usedTmdb } = options
  const cardHeight = layoutCardHeight(layout)
  ...
  ctx.fillStyle = TEXT_COLOR
  ctx.font = cardFont(TYPE_SCALE.barChartHeading, 'bold')
  ctx.textAlign = 'center'
  ctx.textBaseline = 'top'
  ...
  for (const bar of bars) {
    // Label
    ctx.fillStyle = TEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.barChartLabel)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
    ...
    // Count on far right
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.barChartCount)
    ctx.textAlign = 'right'
...
async function renderMilestonesCard(
  ...
  const { username, footerAvatarDataUrl, showShareIcon, backdropDataUrl, milestonesData, usedTmdb } = options
  ...
  ctx.fillStyle = TEXT_COLOR
  ctx.font = cardFont(TYPE_SCALE.milestoneHeading, 'bold')
  ctx.textAlign = 'center'
  ctx.textBaseline = 'top'
  ...
  for (let i = 0; i < count; i++) {
    ...
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.milestoneLabel, 'bold')
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
    ...
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.milestoneMeta)
    ctx.textAlign = 'center'
    ctx.textBaseline = 'top'
...
export async function renderCard(options: CardOptions): Promise<Blob> {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (showTitle) {
        drawMetaLine(cfg.titleH, () => {
          ctx.fillStyle = TEXT_COLOR
          ctx.font = cardFont(cfg.titleFs, 'bold')
          ctx.textAlign = 'left'
          ctx.textBaseline = 'top'
      ...
      if (showRating && film.rating) {
        drawMetaLine(cfg.metaH, () => {
          ...
          ctx.font = cardFont(cfg.metaFs + 3)
          ctx.textAlign = 'left'
          ctx.textBaseline = 'top'
      ...
      if (showOverview && film.overview) {
        if (!firstMeta) contentY += cfg.metaGap
        ctx.fillStyle = SUBTEXT_COLOR
        ctx.font = cardFont(cfg.reviewFs, 'italic')
        ctx.textAlign = 'left'
        ctx.textBaseline = 'top'
      ...
      if (film.reviewText) {
        ...
        ctx.fillStyle = TEXT_COLOR
        ctx.font = cardFont(cfg.reviewFs)
        ctx.textAlign = 'left'
        ctx.textBaseline = 'top'
  ...
  if (showDate && cardType !== 'recent-diary') {
    ...
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.headerDate)
    ctx.textAlign = 'right'
    ctx.textBaseline = 'middle'
  ...
  if (showingListTitle && listTitle) {
    ctx.fillStyle = TEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.listTitle, 'bold')
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
  ...
  if (showingListDesc && listDescription) {
    ...
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.listDescription)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
  ...
  if (showingCardTypeLabel && cardTypeLabel) {
    ctx.fillStyle = TEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.cardTypeLabel, 'bold')
    ctx.textAlign = 'left'
    ctx.textBaseline = 'top'
  ...
  // ── Footer ────────────────────────────────────────────────
  await drawFooter(ctx, layout.footerY, layout.cardWidth, username, footerAvatarDataUrl, showShareIcon, usedTmdb)
...
async function drawFooter(
  ...
  usedTmdb: boolean | undefined = false,
): Promise<void> {
  const SHARE_SIZE = 24
  ...
  if (username) {
    ...
    ctx.fillStyle = SUBTEXT_COLOR
    ctx.font = cardFont(TYPE_SCALE.footerUsername)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'middle'
  ...
  ctx.fillStyle = DIM_COLOR
  ctx.font = cardFont(TYPE_SCALE.footerAttribution)
  ctx.textAlign = 'right'
  ctx.textBaseline = 'middle'
  const attributionText = 'generated by Boxd Card'
  const attributionWidth = ctx.measureText(attributionText).width
  ctx.fillText(attributionText, cardWidth - 40, footerY)

  // TMDB attribution logo — placed to the left of "generated by Boxd Card" with padding
  if (usedTmdb) {
    const tmdbRightX = cardWidth - 40 - attributionWidth - TMDB_LOGO_PAD
    await drawTmdbLogo(ctx, tmdbRightX, footerY)
  }
}
